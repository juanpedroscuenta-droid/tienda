        ...
        conn.exec(`cat > /root/whatsapp_agent/.env << 'EOF'\n${envContent}\nEOF`, (err, stream) => {
            ...
            stream.on('close', () => {
                ...
                conn.sftp((err, sftp) => {
                    ...
                    sftp.fastPut(path.join(__dirname, 'index.js'), '/root/whatsapp_agent/index.js', (err) => {
                        if (err) throw err;
                        console.log('✅ index.js updated!');
                        
                        // 3. Fresh Start PM2
                        console.log('🔄 Performing FRESH service restart...');
                        conn.exec('pm2 delete whatsapp_agent || true && pm2 delete whatsapp || true && cd /root/whatsapp_agent && pm2 start index.js --name "whatsapp_agent" && pm2 save', (err, stream) => {
                            if (err) throw err;
                            stream.on('close', (code) => {
                                console.log(`🚀 DEPLOYMENT FINISHED! (Code: ${code})`);
                                conn.end();
