...
app.post('/webhook', (req, res) => {
    ...
    if (body.object === 'whatsapp_business_account') {
        ...
        if (body.entry &&
            ...
            // --- SYNC WITH CRM PANEL (api-crm-wpp) ---
            axios.post('http://api-crm-wpp:3005/webhook', body).catch(() => {});
            ...
            if (isPaymentText && !isImage && !isAudio) {
                 ...
                 axios.post('http://api-crm-wpp:3005/api/crm/chats/intervention/' + senderId, { 
                    ...
                    if (isImage && mediaId) {
                        ...
                        if (identification === 'NOT_PART') {
                            ...
                            if (session.strikes >= 2) {
                                session.transferred = true; 
                                axios.post('http://api-crm-wpp:3005/api/crm/chats/intervention/' + senderId, { reason: "Cliente envió imagen no reconocida 2 veces." }).catch(() => {});
...
async function processAgentLogic(from, text, isImage = false, isAudio = false) {
    ...
    try {
        ...
        // --- BÚSQUEDA SEMÁNTICA INTELIGENTE (VECTOR SEARCH) Con Fallback a Keyword ---
        try {
            let searchPerformed = false;
            
            if (text.length > 3) {
                try {
                    console.log(`🔎 Intentando búsqueda semántica: "${text}"...`);
                    ...
                    if (!vError && vProducts && vProducts.length > 0) {
                        console.log(`✅ Éxito: Encontrados ${vProducts.length} producto(s) por similitud semántica.`);
                        productContext = "\n\nINVENTARIO REAL (Usa SOLO estos datos, NO inventes precios ni productos):\n";
                        vProducts.forEach(p => {
                            productContext += `- PRODUCTO: ${p.name} | PRECIO: $${Number(p.price).toLocaleString('es-CO')} | STOCK: ${p.stock} unidades\n`;
                            if (p.image) {
                                let fullImageUrl = p.image.startsWith('http') ? p.image : `${SUPABASE_URL}/storage/v1/object/public/24/${p.image}`;
                    ...
                    console.warn('⚠️ Búsqueda semántica falló (Quota Gemini?), usando búsqueda tradicional...');
                }
            }

            // --- FALLBACK: Búsqueda tradicional por palabras clave si la semántica falló o no encontró nada ---
            if (!searchPerformed && searchWords.length > 0) {
                console.log(`🔍 Usando búsqueda tradicional por palabras: ${searchWords.join(', ')}`);
                for (const word of searchWords) {
                    if (productContext) break;
                    const { data: kProducts } = await supabase
                        .from('products')
                        .select('name, price, stock, image')
                        .ilike('name', word.length < 4 ? `${word} %` : `%${word}%`)
                        .eq('is_published', true)
                        .limit(5);

                    if (kProducts && kProducts.length > 0) {
                        console.log(`✅ Búsqueda tradicional encontró ${kProducts.length} producto(s) para "${word}"`);
                        productContext = "\n\nINVENTARIO REAL (Usa SOLO estos datos, NO inventes precios ni productos):\n";
                        kProducts.forEach(p => {
                            const hasImage = p.image ? "Sí" : "No";
                            productContext += `- PRODUCTO: ${p.name} | PRECIO: $${Number(p.price).toLocaleString('es-CO')} | STOCK: ${p.stock} unidades | TIENE IMAGEN: ${hasImage}\n`;
                            if (p.image) {
                                let fullImageUrl = p.image.startsWith('http') ? p.image : `${SUPABASE_URL}/storage/v1/object/public/24/${p.image}`;
                                productImages.push({ url: fullImageUrl, name: p.name });
                            }
            ...
            console.error('❌ Error general en búsqueda:', generalSearchErr.message);
        }

        if (searchWords.length === 0) {
            console.log('ℹ️ Solo palabras comunes en el mensaje, sin búsqueda en BD.');
        } else if (!productContext) {
            console.log('ℹ️ No se encontró nada en la BD para este mensaje.');
        }

        // Add to history
        if (!userSessions[from].history) userSessions[from].history = [];
        userSessions[from].history.push({ role: "user", content: text });
        if (userSessions[from].history.length > 10) userSessions[from].history.shift();

        const systemMessage = `IDENTIDAD (INAMOVIBLE): Te llamas Sebas, experto en REPUESTOS AUTOMOTRICES GENERALES en "R.Repuestos 24/7". 🇨🇴

PRIORIDAD: CIERRE DE VENTA
- Si el cliente quiere comprar: Indica pago a *Nequi 321 2619434* o *QR de Bold*. Pregunta si recoge o envío.

REGLAS DE ORO (MANDATORIAS):
- SALUDO INTELIGENTE: Si el cliente dice "hola" y ya lo saludaste en el historial, responde ÚNICAMENTE: "Dime", "Sí, cuéntame" o "Aquí sigo". NO repitas tu nombre ni el del local.
- SI NO LO TIENES, SÉ SERVICIAL: Si no hay stock, dile amablemente que no lo tienes, pero recomiéndale buscar en sectores conocidos como el *7 de Agosto* o *La Estanzuela* en Bogotá, o en almacenes de marca autorizados. Sé un aliado para el cliente.
- BREVEDAD: Sé muy corto y directo. Máximo 2 párrafos.
...
${productContext}`;
        ...
        while (retries > 0) {
            try {
                aiResponse = await axios.post('https://api.deepseek.com/chat/completions', {
                    model: "deepseek-chat",
                    messages: [{ role: "system", content: systemMessage }, ...userSessions[from].history]
                }, {
        ...
        (async () => {
            try {
                ...
                // 1. Detección por palabras clave de pago
                if (lowerInput.includes("pagué") || lowerInput.includes("comprobante") || lowerInput.includes("transferencia") || lowerInput.includes("voucher") || lowerInput.includes("recibo")) {
                ...
                if (needsIntervention) {
                    ...
                    userSessions[from].transferred = true; // SILENCIAR AL BOT
                    await axios.post('http://api-crm-wpp:3005/api/crm/chats/intervention/' + from, { reason });
                    await sendWhatsAppMessage(from, "Entendido. Para darte una mejor atención, te estoy transfiriendo con uno de nuestros asesores humanos. Por favor, espera un momento... 👨‍💻");
                }
            } catch (err) {
    ...
    // Si el agente menciona el QR de pago, enviamos la imagen del QR automáticamente
    const lowerResponse = response.toLowerCase();
    if (lowerResponse.includes("qr") || lowerResponse.includes("bold") || lowerResponse.includes("pago a nequi")) {
        ...
        await sendWhatsAppMessage(from, "📸 Aquí tiene el código QR de Bold para su pago:", qrUrl);
    }
...
async function sendWhatsAppMessage(to, text, imageUrl = null) {
    // --- SYNC WITH CRM PANEL (api-crm-wpp) ---
    axios.post(`http://api-crm-wpp:3005/api/crm/chats/${to}/messages`, {
