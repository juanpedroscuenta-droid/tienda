...
import { fetchChatBotSettings } from '@/lib/api';
...
export const Footer: React.FC = () => {
  ...
  // Comprobar si hay un bot activo para posicionar el carrito en móviles
  useEffect(() => {
    ...
    <footer className="w-full bg-[#111111] text-white pt-10">
      ...
      <div className="bg-white py-6">
        <div className="max-w-[1200px] mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-6">
          <span className="text-xs font-bold text-gray-500 uppercase tracking-widest text-center md:text-left">Medios de pago y sitio seguro</span>
          <div className="flex flex-wrap items-center justify-center gap-6 md:gap-8 transition-all duration-500">
            <div className="h-8 flex items-center justify-center overflow-hidden mix-blend-multiply transition-opacity">
              <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSbxOyHUCxA_fZ5Llm-z0ajhbPMxIjXMtMAsA&s" alt="Nequi" className="h-full object-contain" />
            </div>
            <div className="h-8 flex items-center justify-center overflow-hidden mix-blend-multiply transition-opacity">
              <img src="https://colombiafintech.co/wp-content/uploads/2025/03/NM_DaviPlata_Color.png" alt="DaviPlata" className="h-full object-contain pt-1" />
            </div>
            <div className="h-8 flex items-center font-black text-gray-500 text-sm uppercase tracking-wider">Transferencias</div>
            <div className="h-8 flex items-center justify-center overflow-hidden mix-blend-multiply transition-opacity">
              <img src="https://bogota.gov.co/sites/default/files/styles/1050px/public/2020-04/logo-bancolombia-2.jpg" alt="Bancolombia" className="h-full object-contain" />
            </div>
            <div className="h-8 flex items-center justify-center overflow-hidden mix-blend-multiply transition-opacity">
              <img src="https://datafonos.bold.co/wp-content/uploads/2022/12/Logo-Bold-Principal.png" alt="Bold" className="h-full object-contain" />
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Floating Cart Button (Mobile Only) - Positioned above the bot or whatsapp */}
      <button
        onClick={() => navigate('/cart')}
        className={`fixed ${hasBot ? 'bottom-[140px]' : 'bottom-[84px]'} right-6 z-50 md:hidden bg-white text-black p-2.5 rounded-full shadow-[0_4px_20px_rgba(0,0,0,0.25)] hover:scale-110 active:scale-95 transition-all flex items-center justify-center h-[48px] w-[48px] border-2 border-slate-900 group`}
      >
        <div className="relative">
          <ShoppingCart className="w-6 h-6" />
          {itemCount > 0 && (
            <span className="absolute -top-3 -right-3 bg-red-600 text-white text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center border-2 border-white shadow-sm animate-in zoom-in-50 duration-300">
              {itemCount}
            </span>
          )}
        </div>
